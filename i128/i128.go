...
func (i I128) String() string {
	// Handle special cases of zero and negative sign
	if i.hi == 0 {
		if i.lo == 0 {
			return "0"
		} else {
		...
		// Convert the absolute value to a string without leading zeros
		absHi := strconv.FormatUint(^i.hi, 10)
		absLo := strconv.FormatUint(^i.lo+1, 10)
		if len(absHi) > 0 && absHi[0] == '0' {
			absHi = absHi[1:]
		}
		return "-" + absHi + absLo
	}
	...
	// General case
	return strconv.FormatUint(i.hi, 10) + strconv.FormatUint(i.lo, 10)
}
